...
const command: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("location")
        .setDescription("location for air quality")
        .setRequired(true),
    ),
  async execute(interaction) {
    ...
    if (location && location.length > 0) {
      const data = (await fetch(
        ...
        data: {
          time: Array<{
            from: string;
            to: string;
            variables: { AQI: { value: number; units: string } };
          }>;
        };
      };
      ...
      if (data && data.data && data.data.time && data.data.time.length > 0) {
        ...
        const level = data.data.time.find((d) =>
          isAfter(new Date(), parseISO(d.from)),
        )!.variables.AQI.value;
        const city =
        ...
        const desc = description.variables.AQI.aqis.find(
          (d) =>
            level >= (d.from ?? 0) &&
            level <= (d.to ?? Number.MAX_SAFE_INTEGER),
        );

        if (desc) {
          await interaction.reply(
            `${city}: ${desc.description_EN} (${desc.short_description_EN})`,
          );
        } else {
